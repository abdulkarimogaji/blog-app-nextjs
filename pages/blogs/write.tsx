...
const Create = () => {
  ...
  const handleSubmit = async (e: any) => {
    ...
    for (let i = 1; i < sectionCount + 1; i++) {
      ...
      const singleSection = {
        title: e.target.elements[`sectionTitle${i}`].value,
        content: e.target.elements[`sectionContent${i}`].value,
        image,
      };
    ...
    <div className=" md:p-16 p-4">
      ...
      <form className="md:w-4/5 container my-center" onSubmit={handleSubmit}>
        <section className="bg-white md:p-8 p-4 border border-gray-300 rounded-lg my-8">
          ...
          <h2 className="md:text-lg text-base md:text-2xl md:mb-16 mb-8">
            Introduction
          </h2>
          <div className="my-12">
          ...
          <div className="my-12">
            ...
            <input
              type="text"
              placeholder="Seperate tags with comma e.g politics, sports"
              name="tags"
              className="border hover:border-gray-600 focus:border-gray-600 container rounded-lg p-2 pe-5 md:text-base text-sm outline-none"
            />
          </div>
          ...
          <div className="my-12">
            <p className="md:text-lg text-base font-semibold md:mb-8 mb-4">
              Cover Image
            </p>
            <input type="file" name="fileIntro" />
          </div>
        </section>
        {allSections.map((_v, idx) => (
          <BlogSectionForm key={idx + 1} secNum={idx + 1} />
        ))}
        <button
          type="button"
          className="block action-btn2 text-xs md:text-sm p-2 px-4 rounded-md mb-4"
          onClick={addSection}
        >
          Add Another Section
        </button>
