...
const BlogDetail = () => {
  ...
  const { mutate } = useMutation<MyResponseType<Comment>, any, any>(
    sendComment,
    {
    ...
    return (
      <>
        <Script id="details-script">
          {document.addEventListener("load", () => analyse("blog-details"), {
            once: true,
          })}
        </Script>
        <section className=" bg-white">
          ...
          <div className="text-center">
            ...
            <Avatar
              dimension="w-20 h-20 mb-2"
              href={`/users/${blog.author._id}`}
              src={blog.author.picture}
            />
            <div className="text-sm mb-4">
              BY {blog.author.username}, UPDATED ON{" "}
              {dateToMonthDay(blog.updatedAt)}
            </div>
          ...
          <article className="flex-center">
            <div className="w-4/5">
              ...
              {blog.sections.map((sec, i) => (
                <section
                  className="md:my-16 my-8 md:px-32 px-4"
                  id={sec.title}
                  key={sec.title}
                >
                  {sec.title && (
                    <h2 className="md:text-5xl text-lg md:mb-8 mb-4">
                      {sec.title}
                    </h2>
                  )}
                  ...
                  <div className="flex-center md:my-16 my-8">
                    ...
                    {sec.image ? (
                      <div className="relative md:w-3/4 w-4/5 md:h-96 h-40">
                        <Image src={sec.image} alt="example" layout="fill" />
                      </div>
                    ) : (
                      <br />
                    )}
              ...
              <div className="flex-center">
                <div className="rounded-lg bg-gray-200 md:w-2/3 container md:p-8 p-3 md:text-sm text-xs flex md:gap-6 gap-3">
                  <Avatar
                    dimension="md:w-20 md:h-20 h-10 w-10"
                    href={`/users/${blog.author._id}`}
                    src={blog.author.picture}
                  />
                  <div>
                    ...
                    <p>
                      {blog.author.username} &bull; Joined at{" "}
                      {dateToMonthDay(blog.author.createdAt)}
                    </p>
                    <br />
                    <br />
              ...
              <div className="flex flex-col items-center md:my-32 my-16">
                <h1 className="md:text-3xl text-lg mb-16">
                  ...
                  <div className="rounded-lg md:w-2/3 container md:p-8 p-3 md:text-sm text-xs flex md:gap-6 gap-3">
                    <div className="flex-center">
                      <div className="relative md:w-15 md:h-15 h-7 w-7">
                        <Image
                          src={userData.picture || "/default_avatar.jfif"}
                          alt="default_avatar"
                          layout="fill"
                    ...
                    <div className="container">
                      ...
                      <button
                        type="submit"
                        onClick={addComment}
                        className="md:p-2 p-1 md:px-4 px-2 text-xs md:text-sm action-btn rounded-lg block my-2"
                      >
                        Submit
                      </button>
                    </div>
                ...
                {blog.comments.map((com) => (
                  <SingleComment key={com._id} data={com} />
                ))}
              </div>
            </div>
          </article>
